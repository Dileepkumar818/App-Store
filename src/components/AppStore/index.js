...
class AppStore extends Component {
  state = {activeTab: tabsList[0].tabId, searchValue: ''}
  ...
  render() {
    ...
    const allApps = appsList.filter(each => activeTab === each.category)
    const apps = allApps.filter(each =>
    ...
    return (
      <div className="bg-container">
        ...
        <ul className="tab-container">
          {tabsList.map(each => (
            <TabItem item={each} key={each.tabId} getItems={this.getItems} />
          ))}
        </ul>

        <ul className="app-container">
          {apps.map(each => (
            <AppItem items={each} key={each.appId} />
          ))}
        </ul>
      </div>
    )
